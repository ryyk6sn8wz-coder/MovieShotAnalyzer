...
class MovieShotAnalyzer(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle('Movie Shot Analyzer — v2.0.8 XZ Axis Isolation'); self.resize(1500,920); self.setMinimumSize(1050,680); self.setAcceptDrops(True)
    ...
    def solve_perspective_axis(self,name):
        # v2.0.8 axis-transaction guard:
        # Solving one horizontal axis must never rewrite the already-confirmed
        # geometry/state of the other axis.  This is intentionally defensive:
        # lens recomputation may read X+Z, but axis solving owns only `name`.
        other = 'vp2' if name == 'vp1' else ('vp1' if name == 'vp2' else None)
        other_snapshot = None
        if other is not None:
            other_snapshot = {
                'vp': tuple(self.vp1 if other == 'vp1' else self.vp2),
                'lines': copy.deepcopy(self.perspective_lines.get(other, [])),
                'extra': copy.deepcopy(self.perspective_extra_lines.get(other, [])),
                'complete': bool(self._persp_axis_complete.get(other, False)),
                'infinity': bool(self.vp_at_infinity.get(other, False)),
                'inf_dir': copy.deepcopy(self.camera_inf_dir.get(other)),
                'locked': copy.deepcopy(self._persp_line1_locked.get(other)),
                'touch0': copy.deepcopy(self._persp_anchor_touched.get((other,0), set())),
                'touch1': copy.deepcopy(self._persp_anchor_touched.get((other,1), set())),
            }
        ...
        if name=='vp3':
            ...
            # Restore the opposite axis byte-for-byte before the shared lens reader
            # runs.  Therefore placing Z cannot move/reset X (and vice versa).
            if other_snapshot is not None:
                if other == 'vp1': self.vp1=other_snapshot['vp']
                else: self.vp2=other_snapshot['vp']
                self.perspective_lines[other]=copy.deepcopy(other_snapshot['lines'])
                self.perspective_extra_lines[other]=copy.deepcopy(other_snapshot['extra'])
                self._persp_axis_complete[other]=other_snapshot['complete']
                self.vp_at_infinity[other]=other_snapshot['infinity']
                self.camera_inf_dir[other]=copy.deepcopy(other_snapshot['inf_dir'])
                self._persp_line1_locked[other]=copy.deepcopy(other_snapshot['locked'])
                self._persp_anchor_touched[(other,0)]=copy.deepcopy(other_snapshot['touch0'])
                if other_snapshot['touch1']:
                    self._persp_anchor_touched[(other,1)]=copy.deepcopy(other_snapshot['touch1'])
                else:
                    self._persp_anchor_touched.pop((other,1),None)
            self.recompute_lens_from_current_vps()
